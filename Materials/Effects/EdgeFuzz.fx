...
float4 pixel_shader(VS_OUTPUT IN) : SV_Target
{
	...
	float hdn = dot(halfN, IN.WorldNormal);
	float4 lv = lit(ldn, hdn, SpecExpon);

	float subd = abs(dot(IN.WorldNormal, IN.WorldView));
	...
	float3 IncidentColor = lerp(EdgeColor, Lamp0Color, ldn);
	IncidentColor = lerp(Lamp0Color, IncidentColor, EdgeFadeL);
